    ...
    public class SharesDividendClass(IDbContextFactory<ApplicationDbContext> dbFactory, ILogger<SharesDividendClass> logger) : ISharesDividendClass
    {
        ...
        public async Task<(double TotalMoneyProfitOrLoss, int Count)> GetTotalMoneyProfitOrLossAndCount()
        {
            try
            {
                ...
                var result = await db.SharesDividends
                    .GroupBy(x => 1)
                    .Select(g => new
                    {
                        TotalMoneyProfitOrLoss = g.Sum(x => x.TotalAmount),
                        Count = g.Count()
                    })
                    .FirstOrDefaultAsync();
            ...
            {
                _logger.LogError(ex, "Could not get total MoneyProfitOrLoss and count from SharesDividends.");
                return (0, 0);
            }
        }
    }
