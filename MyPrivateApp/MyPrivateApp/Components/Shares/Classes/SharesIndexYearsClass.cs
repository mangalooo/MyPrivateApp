    ...
    public class SharesIndexYearsClass(IDbContextFactory<ApplicationDbContext> dbFactory, ILogger<SharesIndexYearsClass> logger) : ISharesIndexYearsClass
    {
        ...
        public async Task<string> CalculateLastYearsResults()
        {
            try
            {
                await using ApplicationDbContext db = _dbFactory.CreateDbContext()
                    ?? throw new Exception("CalculateLastYearsResults: db == null!");
                ...
                List<string> yearStrings = await db.SharesProfitOrLossYears
                    .AsNoTracking()
                ...
                int thisYear = 2010; //DateTime.Now.Year; // Magnus: Ändra tillbaka när alla år är uppdaterat annars justera här.
        ...
        public async Task<string> Delete(SharesProfitOrLossYears model)
        {
            try
            {
                ...
                _dbFactory.CreateDbContext() ?? throw new Exception("Delete: db == null!");
        ...
        public async Task<SharesTotalProfitsOrLosses> GetTotalProfitsOrLosses(int? id)
        {
            ...
            await using ApplicationDbContext db = _dbFactory.CreateDbContext()
                ?? throw new Exception("GetTotalProfitsOrLosses: db == null!");
